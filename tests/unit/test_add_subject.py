import pytest
from app.logic.add_subject import process_pdf_files, update_index_html, add_new_subject
import os
import tempfile
...
def test_update_index_html_inserts(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        monkeypatch.setattr("app.logic.add_subject.INDEX_HTML_PATH", index_path)
...
def test_update_index_html_no_duplicate(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(index_path, "r", encoding="utf-8") as f:
            ...
            assert content.count(subject_name) == 1
...
def test_add_new_subject_calls(monkeypatch):
    ...
    def fake_update_index_html(name):
        called["update"] = name
    monkeypatch.setattr("app.logic.add_subject.update_index_html", fake_update_index_html)
